...
export default async function activate(pi) {
    ...
    function buildFilteredContent(originalContent, command) {
        // Find the original text output
        const originalTextItems = originalContent.filter((c) => c.type === 'text');
    ...
    pi.on('tool_result', (async (event, _ctx) => {
        ...
        const textItems = event.content.filter((c) => c.type === 'text');
